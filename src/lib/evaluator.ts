...
import {ConflictState, PackageDetails, PackageRequirement, ResolvedPackage, State} from './evaluator.interface';
    ...
    if (openRequirements.length) {
      ...
      try {
        const availableVersions = version
          ? [version]
          : (await this.client.getAllVersionsFromRegistry(currentRequirement.name)).sort(compareVersions).reverse();
        ...
        for (const versionToExplore of compatibleVersions) {
          if (conflictState.state === State.CONFLICT) {
            ...
            conflictState = await this.evaluationStep(
              currentRequirement.peer && !selectedPackageVersions.some((rp) => rp.name === packageDetails.name && rp.name === packageDetails.name)
